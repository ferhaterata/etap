...
icmp <- function(cond, left, right) {
  ...
  # --------------------------------------
  cmp <- left - right; cmp.true <- 1; cmp.false <- 0
...
br <- function(iftrue, iffalse, cond) {
  if (missing(iffalse)) { #unconditional branch
    eval.parent(str2expression(paste0(.block$name, "_", iftrue, " <- round(1.00)")))
  } else { #conditional branch
    eval.parent(str2expression(paste0(.block$name, "_", iftrue, " <- round(", cond[1], ", 4)")))
    eval.parent(str2expression(paste0(.block$name, "_", iffalse, " <- round(1 - ", cond[1], ", 4)")))
  }
