...
block.entry <- env(
  ...
  insts = list(
    list("p.retval <- alloca(\"int\")", "nop"),
    list("p.n.addr <- alloca(\"int\")", "nop"),
    list("p.x <- alloca(\"int\")", "nop"),
    list("p.xp <- alloca(\"int*\")", "nop"),
    ...
    list("v.cmp <- icmp(sgt, v.1, 1)", "v.1 <- add(v.cmp, 1); 1 <- sub(v.1, v.cmp"),
    list("br(quote(if.then), quote(if.end3), v.cmp)", "nop")))
...
block.if.then <- env(
  ...
  insts = list(
    list("if (entry) rollback(v.cmp.true)", "nop"),
    list("v.2 <- load(p.n.addr)", "store(v.2, p.n.addr)"),
    list("v.cmp1 <- icmp(sgt, v.2, 2)", "v.2 <- add(v.cmp1, 2)"),
    list("br(quote(if.then2), quote(if.end), v.cmp1)", "nop")))
...
block.if.end <- env(
  ...
  succs = list("if.end3"),
  insts = list(
    "if.end_if.end3 <- 1.00; if.end <- T; if.then <- F"))
...
block.if.end3 <- env(
  ...
  succs = list("return"),
  insts = list(
    list("if (entry) rollback(v.cmp.false)", "noinverse"),
    "p.4 <- load(p.xp)",
    "v.5 <- load(p.4)",
    "store(v.5, p.retval)",
    "if.end3_return <- 1.00; if.end3 <- T; if.end <- F; entry <- F"))
...
block.return <- env(
  ...
  insts = list(
    "v.6 <- load(p.retval)",
    "path.return <<- c(path.return, ret(v.6)); return <- T; if.end3 <- F; if.then2 <- F"))
...
.function <- list(
  ...
  initials = list("entry <- F; if.then <- F; if.then2 <- F; if.end <- F; if.end3 <- F; return <- F"),
  globals = list(),
  ...
  for (i in seq_along(succs[[v$name]])) {
    ...
    if (length(succs[[v$name]][[i]])) {
      ...
      if (value > 0 && all(visited <= max_loop)) {
        ...
        # else
        clone <- rlang::env_clone(env) # don't create unnecessary environments.
...
# run the machine
DFS(list(), .function$blocks[[1]], "", "1", visited, edge_visited, env)
