...
block.if.then <- env(
  ...
  insts = list(
    list("if(entry) {sym$\"cmp\" <- sym$\"cmp.true\"; {if (is(sym$\"0\", \"Distribution\")) sym$\"0\" <- add(sym$\"cmp\", 21)};store(sym$\"0\", sym$\"data.addr\"); };", "nop", "0", "0"),
...
block.if.else <- env(
  ...
  insts = list(
    list("if(entry) {sym$\"cmp\" <- sym$\"cmp.false\"; {if (is(sym$\"0\", \"Distribution\")) sym$\"0\" <- add(sym$\"cmp\", 21)};store(sym$\"0\", sym$\"data.addr\"); };", "nop", "0", "0"),
...
block.if.end <- env(
  name = "if.end",
  preds = list("if.else3", "checkpoint.2"),
  succs = list("if.end4"),
...
block.if.end4 <- env(
  name = "if.end4",
  preds = list("if.end", "checkpoint.1"),
  succs = list(),
  energy = "E(classify, if.end4)",
  timing = "T(classify, if.end4)",
  insts = list(
    list("sym$\"3\" <- load(sym$\"result\")", "nop", "E(load)", "T(load)"),
    list("path.return <<- c(path.return, ret(sym$\"3\")); if.end4 <- TRUE; if.end <- FALSE; checkpoint.1 <- FALSE", "nop", "E(ret)", "T(ret)")))

preds <- list(list(), list(block.entry), list(block.if.then), list(block.entry), list(block.if.else), list(block.if.then2), list(block.if.else), list(block.if.else3, block.checkpoint.2), list(block.if.end, block.checkpoint.1))
names(preds) <- c(block.entry$name, block.if.then$name, block.checkpoint.1$name, block.if.else$name, block.if.then2$name, block.checkpoint.2$name, block.if.else3$name, block.if.end$name, block.if.end4$name)

succs <- list(list(block.if.then, block.if.else), list(block.checkpoint.1), list(block.if.end4), list(block.if.then2, block.if.else3), list(block.checkpoint.2), list(block.if.end), list(block.if.end), list(block.if.end4), list())
names(succs) <- c(block.entry$name, block.if.then$name, block.checkpoint.1$name, block.if.else$name, block.if.then2$name, block.checkpoint.2$name, block.if.else3$name, block.if.end$name, block.if.end4$name)

edge_global <- as.list(rep(0, 10))
edge_local <- as.list(rep(0, 10))
edge_visited <- as.list(rep(0, 10))
names(edge_global) <- c("entry#if.then", "entry#if.else", "if.then#checkpoint.1", "checkpoint.1#if.end4", "if.else#if.then2", "if.else#if.else3", "if.then2#checkpoint.2", "checkpoint.2#if.end", "if.else3#if.end", "if.end#if.end4")
names(edge_local) <- c("entry#if.then", "entry#if.else", "if.then#checkpoint.1", "checkpoint.1#if.end4", "if.else#if.then2", "if.else#if.else3", "if.then2#checkpoint.2", "checkpoint.2#if.end", "if.else3#if.end", "if.end#if.end4")
names(edge_visited) <- c("entry#if.then", "entry#if.else", "if.then#checkpoint.1", "checkpoint.1#if.end4", "if.else#if.then2", "if.else#if.else3", "if.then2#checkpoint.2", "checkpoint.2#if.end", "if.else3#if.end", "if.end#if.end4")

freq <- as.list(rep("0", 9))
names(freq) <- c(block.entry$name, block.if.then$name, block.checkpoint.1$name, block.if.else$name, block.if.then2$name, block.checkpoint.2$name, block.if.else3$name, block.if.end$name, block.if.end4$name)

visited <- as.list(rep(0, 9))
names(visited) <- c(block.entry$name, block.if.then$name, block.checkpoint.1$name, block.if.else$name, block.if.then2$name, block.checkpoint.2$name, block.if.else3$name, block.if.end$name, block.if.end4$name)

color <- as.list(rep("white", 9))
names(color) <- c(block.entry$name, block.if.then$name, block.checkpoint.1$name, block.if.else$name, block.if.then2$name, block.checkpoint.2$name, block.if.else3$name, block.if.end$name, block.if.end4$name)

total_visit <- as.list(rep(0, 9))
names(total_visit) <- c(block.entry$name, block.if.then$name, block.checkpoint.1$name, block.if.else$name, block.if.then2$name, block.checkpoint.2$name, block.if.else3$name, block.if.end$name, block.if.end4$name)

.function <- list(
  name = "classify",
  formals = list("sym$\"data\" <- Mixing(Binom(40, 0.4), 15 + Binom(30, 0.6), mixCoeff = c(0.7, 0.3))"),
  initials = list("entry <- FALSE; if.then <- FALSE; checkpoint.1 <- FALSE; if.else <- FALSE; if.then2 <- FALSE; checkpoint.2 <- FALSE; if.else3 <- FALSE; if.end <- FALSE; if.end4 <- FALSE"),
  globals = list(),
  types = list(),
  blocks = list(block.entry, block.if.then, block.checkpoint.1, block.if.else, block.if.then2, block.checkpoint.2, block.if.else3, block.if.end, block.if.end4))

# Capacitor Thresholds
min_cap <- 520200 * 2 #nj
max_cap <- 781250 * 2 #nj

# 2 mF capacitance 40 cm distance profile
...
lab <- paste0("95% Confidence Interval (", rnd[1], ", ", rnd[2], ", ", rnd[3], ")")
################################################
timingPlot <- paste0("Plots/", .function$name, ".intermittent.2mF.1MHz.Variant3.pdf")
dir.create(dirname(timingPlot), showWarnings = FALSE, recursive = TRUE)
...
plot(path.timing.mixture, font = 2, font.lab = 2, font.main = 2, cex = 1.1, fontfamily = "Fira Sans", mfColRow = FALSE, to.draw.arg = "d", bty = "7",
     ...
     col.points = c("red", "black"), cex.points = 1.8, ylab = "", inner = "", xlim = c(10000, 80000)
     # inner = list(paste("Probability Function Of", .function$name))
)
