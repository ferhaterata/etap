...
`cppFunction` allows you to write an inline C++ function in R:
...
Rcpp::cppFunction('
long trunc(String from, long x, String to) {
  if (to == "i1")
    return x & 1;
  if (to == "i8")  // char
    return (char)x;
  ...
  std::string msg =std::string("\\\'to\\\' type is not recognized: ") + to.get_cstring() + std::string(" \\n");
  Rcpp::warning(msg);
  return x;
 }
')
