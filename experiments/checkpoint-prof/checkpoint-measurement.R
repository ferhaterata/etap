...
energy_trace <- read.csv("experiments/checkpoint-prof/.energytrace/16MHz_100forCheckpoint_EnergyTrace_2021_08_01_103238.csv", header = TRUE, sep = ";")
...
for (i in 1:BLOCK_COUNT) {
  ...
  population_distr <- population_distr / 1000 # ns to us
...
for (i in 1:BLOCK_COUNT) {
  ...
  population_distr <- distr::Norm(x_bar, se)
  population_distr<-population_distr/PRECISION_LOOP_COUNT # for 1 checkpoint
  population_distr <- population_distr * 1000
  plot(population_distr, to.draw.arg = "d", bty = "7",
       panel.first = grid(lwd = 1), lwd = 1, vertical = FALSE,
       col.points = c("red", "black"), cex.points = 1.8, col = "red")
...
print_addressing_modes_time <- function() {
  cat("\nTime @ 1 MHz (us)")
  cat("\nMode ", "Mean", "Sd")
  cat("\nCheckpoint ", round(mean(block_time_dist_list[[1]]), 2), round(sd(block_time_dist_list[[1]]), 2))
}

print_addressing_modes_energy <- function() {
  cat("\nEnergy (nj)")
  cat("\nMode ", "Mean", "Sd")
  cat("\nCheckpoint ", round(mean(block_energy_dist_list[[1]]), 2), round(sd(block_energy_dist_list[[1]]), 2))
}
