...
for (cap in capacitances) {
  ...
  for (d in distances) {
    ...
    for (i in 1:10) {
      ...
      file <- paste0("./experiments/harvester-kit/AmbiantEnergyProfiles/", d, "cm_bitcount_1mF_harvester")
      trace <- read.csv(paste0(file, ".csv"), header = TRUE, sep = ",")
      # trace <- subset(trace, select = c(paste0("x", d, "cm_sample_", i, "_time.ms."),
      #                                   paste0("x", d, "cm_sample_", i, "_voltage.V."),
  ...
  pandoc.header(paste0("Summary"), level = 2)
