...
FORCE_INLINE static void nop_split(const float complex* x, float complex* X,
                                   size_t N) {
...
FORCE_INLINE static void fft_split(const float complex* x, float complex* X,
                                   size_t N, float complex phi) {
  for (size_t n = 0; n < N / 2; n++) {
    X[2 * n + 0] = (x[0 / 2 + n] + x[N / 2 + n]);
    X[2 * n + 1] =
        (x[0 / 2 + n] - x[N / 2 + n]) * cexp(-2 * (float)M_PI * I * phi);
  }
}

FORCE_INLINE static size_t revbits(size_t v, int J) {
  size_t r = 0;
...
FORCE_INLINE static int nop_reverse(int b, float complex* buffers[2],
                                    size_t N) {
  int J = ctz(N);
...
FORCE_INLINE static int fft_reverse(int b, float complex* buffers[2],
                                    size_t N) {
  int J = ctz(N);
...
FORCE_INLINE int fft(float complex* vector, size_t N) {
  if (!N) return 0;
