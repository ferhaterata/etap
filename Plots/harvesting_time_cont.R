...
#                 color = "red", size = 1.5) +
#   xlab("")+ ylab("")+
#   scale_x_continuous(breaks = scales::pretty_breaks(n = 5)) + theme(text = element_text(size=25)) +
...
#                 color = "red", size = 1.5) +
#   xlab("")+ ylab("")+ theme(text = element_text(size=25))  +
...
p1 <- ggplot(histdf_100cm, aes(x = x)) +
  # geom_histogram(binwidth = 0.05,
  #                color = "white", fill = "cornflowerblue", size = 0.1)+
  stat_function(fun = function(x) dnorm(x, mean = mean(data_100cm), sd = sd(data_100cm)) *
    10000 *
    0.05,
                color = "red", size = 1.5) +
  stat_function(fun = function(x) dnorm(x, mean = mean(data_140cm), sd = sd(data_140cm)) *
    10000 *
    0.05,
                color = "purple", size = 1.5) +
  stat_function(fun = function(x) dnorm(x, mean = mean(data_120cm), sd = sd(data_120cm)) *
    10000 *
    0.01,
                color = "blue", size = 1.5) +
  xlab("") +
...
ggsave("harvesting_figure_cont.png", p1, height = 10, width = 15)
