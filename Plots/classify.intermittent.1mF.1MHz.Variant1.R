...
block.if.end4 <- env(
  ...
  insts = list(
    ...
    list("path.return <<- c(path.return, ret(sym$\"3\")); if.end4 <- TRUE; if.end <- FALSE; if.then <- FALSE", "nop", "E(ret)", "T(ret)")))

preds <- list(list(), list(block.entry), list(block.checkpoint.1), list(block.checkpoint.1), list(block.if.else), list(block.if.else), list(block.if.else3, block.if.then2), list(block.if.end, block.if.then))
names(preds) <- c(block.entry$name, block.checkpoint.1$name, block.if.then$name, block.if.else$name, block.if.then2$name, block.if.else3$name, block.if.end$name, block.if.end4$name)

succs <- list(list(block.checkpoint.1), list(block.if.then, block.if.else), list(block.if.end4), list(block.if.then2, block.if.else3), list(block.if.end), list(block.if.end), list(block.if.end4), list())
names(succs) <- c(block.entry$name, block.checkpoint.1$name, block.if.then$name, block.if.else$name, block.if.then2$name, block.if.else3$name, block.if.end$name, block.if.end4$name)

edge_global <- as.list(rep(0, 9))
edge_local <- as.list(rep(0, 9))
edge_visited <- as.list(rep(0, 9))
names(edge_global) <- c("entry#checkpoint.1", "checkpoint.1#if.then", "checkpoint.1#if.else", "if.then#if.end4", "if.else#if.then2", "if.else#if.else3", "if.then2#if.end", "if.else3#if.end", "if.end#if.end4")
names(edge_local) <- c("entry#checkpoint.1", "checkpoint.1#if.then", "checkpoint.1#if.else", "if.then#if.end4", "if.else#if.then2", "if.else#if.else3", "if.then2#if.end", "if.else3#if.end", "if.end#if.end4")
names(edge_visited) <- c("entry#checkpoint.1", "checkpoint.1#if.then", "checkpoint.1#if.else", "if.then#if.end4", "if.else#if.then2", "if.else#if.else3", "if.then2#if.end", "if.else3#if.end", "if.end#if.end4")

freq <- as.list(rep("0", 8))
names(freq) <- c(block.entry$name, block.checkpoint.1$name, block.if.then$name, block.if.else$name, block.if.then2$name, block.if.else3$name, block.if.end$name, block.if.end4$name)

visited <- as.list(rep(0, 8))
names(visited) <- c(block.entry$name, block.checkpoint.1$name, block.if.then$name, block.if.else$name, block.if.then2$name, block.if.else3$name, block.if.end$name, block.if.end4$name)

color <- as.list(rep("white", 8))
names(color) <- c(block.entry$name, block.checkpoint.1$name, block.if.then$name, block.if.else$name, block.if.then2$name, block.if.else3$name, block.if.end$name, block.if.end4$name)

total_visit <- as.list(rep(0, 8))
names(total_visit) <- c(block.entry$name, block.checkpoint.1$name, block.if.then$name, block.if.else$name, block.if.then2$name, block.if.else3$name, block.if.end$name, block.if.end4$name)

.function <- list(
  name = "classify",
  formals = list("sym$\"data\" <- Mixing(Binom(40, 0.4), 15 + Binom(30, 0.6), mixCoeff = c(0.7, 0.3))"),
  initials = list("entry <- FALSE; checkpoint.1 <- FALSE; if.then <- FALSE; if.else <- FALSE; if.then2 <- FALSE; if.else3 <- FALSE; if.end <- FALSE; if.end4 <- FALSE"),
  globals = list(),
  types = list(),
  blocks = list(block.entry, block.checkpoint.1, block.if.then, block.if.else, block.if.then2, block.if.else3, block.if.end, block.if.end4))
...
lab <- paste0("95% Confidence Interval (", rnd[1], ", ", rnd[2], ", ", rnd[3], ")")
################################################
timingPlot <- paste0("Plots/", .function$name, ".intermittent.1mF.1MHz.Variant1-1.svg")
dir.create(dirname(timingPlot), showWarnings = FALSE, recursive = TRUE)
# systemfonts::match_font("Fira Sans", bold = TRUE)
extrafont::loadfonts()
# svg(timingPlot, width = 10.5, height = 3.5)
svg(timingPlot, width = 10.5, height = 3.5, family = "Fira Sans")
par(mfrow = c(1, 3))
plot(path.timing.mixture, font = 2, font.lab = 2, font.main = 2, cex = 1.1, fontfamily = "Fira Sans", mfColRow = FALSE, to.draw.arg = "d", bty = "7",
     panel.first = grid(lwd = 1.1), lwd = 2, vertical = FALSE, xlab = "", cex.axis = 1.2,
     col.points = c("red", "black"), cex.points = 1.8, ylab = "", inner = ""
...
plot(path.timing.mixture, font = 2, font.lab = 2, font.main = 2, cex = 1.1, fontfamily = "Fira Sans", mfColRow = FALSE, to.draw.arg = "q", bty = "7",
     panel.first = grid(lwd = 1.1), lwd = 2, vertical = FALSE, inner = "", cex.axis = 1.2,
     col.points = c("red", "black"), cex.points = 1.8, xlab = "",
     # ylab = "probability",
     # inner = list(paste("CDF of", .function$name))
...
sensor_input <- paste0("Plots/", .function$name, ".sensor_input.svg")
dir.create(dirname(sensor_input), showWarnings = FALSE, recursive = TRUE)
# systemfonts::match_font("Fira Sans", bold = TRUE)
extrafont::loadfonts()
path.timing.mixture <- distr::UnivarMixingDistribution(Binom(40, 0.4), 15 + Binom(30, 0.6), mixCoeff = c(0.7, 0.3))
svg(sensor_input, width = 4.5, height = 5, family = "Fira Sans")
par(mfrow = c(1, 1))
plot(path.timing.mixture, font = 2, font.lab = 2, font.main = 2, cex = 1.1, fontfamily = "Fira Sans", mfColRow = FALSE, to.draw.arg = "d", bty = "7",
     panel.first = grid(lwd = 1.1), lwd = 2, vertical = FALSE, xlab = "", cex.axis = 1.2,
     col.points = c("red", "black"), cex.points = 1.8, ylab = "", inner = ""
