; ModuleID = 'test/r/input/malloc5.ll'
source_filename = "./input/malloc5.c"
...
if.end5:                                          ; preds = %if.then2, %if.end
  %6 = load i32, i32* getelementptr inbounds (%struct.S, %struct.S* @buf, i32 0, i32 0, i64 0), align 4, !store !30
  ret i32 %6, !store !31
}
...
!8 = !{!"store(0, p.retval)"}
!9 = !{!"store(3, p.i)"}
!10 = !{!"store(\\\22null\\\22, p.q)"}
!11 = !{!"v.0 <- load(gep(ptr(c.buf), 0, 0, 1))"}
...
!16 = !{!"store(ptr(c.buf), p.p)"}
!17 = !{!"p.1 <- load(p.p)"}
!18 = !{!"p.data <- gep(p.1, 0, 0)"}
!19 = !{!"p.arrayidx <- gep(p.data, 0, 1)"}
...
!24 = !{!"v.4 <- load(p.3)"}
!25 = !{!"p.5 <- load(p.p)"}
!26 = !{!"p.data3 <- gep(p.5, 0, 0)"}
!27 = !{!"p.arrayidx4 <- gep(p.data3, 0, 0)"}
!28 = !{!"store(v.4, p.arrayidx4)"}
!29 = !{!"%if.then2%if.end5 <- 1.00; if.then2 <- T; if.end <- F"}
!30 = !{!"v.6 <- load(gep(ptr(c.buf), 0, 0, 0))"}
!31 = !{!".ret <<- ret(v.6); if.end5 <- T; if.then2 <- F; if.end <- F"}
