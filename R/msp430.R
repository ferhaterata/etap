...
instruction_parser <- function(line) {
  operand2 <- ""
  operand1 <- ""
  splited_line <- stringr::str_split(line, " ", 2)[[1]]
  instruction <- splited_line[[1]]
  if (length(splited_line) > 1) {
    operands <- splited_line[[2]]
    operand1 <- stringr::str_split(operands, ", ")[[1]][[1]]
    if (grepl(",", line)) {
      operand2 <- stringr::str_split(operands, ", ")[[1]][[2]]
    }
  }
...
readFile <- function(file_name) {
  ...
  for (i in seq_along(file)) {
    file[[i]] <- stringr::str_trim(file[[i]])
  }
  return(file)
}
...
get_cycle_and_energy <- function(instruction) {
  ...
  am_time_map <- list("000" = distr::Norm(1.02, 0.01), # 1 cycle
                      "001" = distr::Norm(3.02, 0.01), # 4
                      "010" = distr::Norm(3.02, 0.01), # 3
                      "011" = distr::Norm(5.02, 0.01), # 6
                      "100" = distr::Norm(2.02, 0.01), # 2
                      "101" = distr::Norm(4.02, 0.01), # 5
                      "110" = distr::Norm(2.02, 0.01), # 2
                      "111" = distr::Norm(4.02, 0.01)) # 5
  ...
  op1 <- instruction$op1
  op2 <- instruction$op2
  if (inst == "") return(list(0, 0))

  if (inst == "call" && op1 == "#__mspabi_mpyi") {
    return(list(distr::Norm(26.016, 0.02), distr::Norm(36.01, 0.47)))
  }
  address_mode_s <- "00"
  address_mode_d <- "0"
  ...
  if (inst == "formatII" ||
    ...
    if (address_mode == "000") {
      return(switch(inst,
                    ...
                    "push" = list(3.01, distr::Norm(7.08, 0.62)),
                    "call" = list(3.02, distr::Norm(8.33, 0.62))))
    }else if (address_mode_s == "01") {
      return(switch(inst,
                    "formatII" = list(4, distr::Norm(8.33, 0.62)),
                    "push" = list(4.02, distr::Norm(10.1, 0.62)),
                    "call" = list(4.02, distr::Norm(10.1, 0.62))))
    }else if (address_mode_s == "10" && grepl("^@r[0-9]{1,2}$", op2)) {
      return(switch(inst,
                    "formatII" = list(3, distr::Norm(7.08, 0.62)),
                    "push" = list(3.52, distr::Norm(8.33, 0.62)),
                    "call" = list(3.02, distr::Norm(8.33, 0.62))))
    }else if (address_mode_s == "11") {
      return(switch(inst,
                    "formatII" = list(3, distr::Norm(7.08, 0.62)),
                    "push" = list(3.02, distr::Norm(8.33, 0.62)),
                    "call" = list(4.02, distr::Norm(10.1, 0.62))))
    }
  }
  ...
  for (f in seq_along(func_blocks_total_cycles_energy)) {
    ...
    for (bb in seq_along(func_blocks_total_cycles_energy[[f]])) {
      ...
      if (is.na(sd(total_cycle))) {
        ...
        time <- paste0("timing$", func_name, "$", bb_name, " <- distr::Norm(mean=", round(mean(total_cycle), 2), ", sd=", round(sd(total_cycle), 2), ") # us\n")
      }
      if (is.na(sd(total_energy))) {
        energy <- paste0("energy$", func_name, "$", bb_name, " <- ", total_energy, " # nj\n")
      }else {
        energy <- paste0("energy$", func_name, "$", bb_name, " <- distr::Norm(mean=", round(mean(total_energy), 2), ", sd=", round(sd(total_energy), 2), ") # nj\n")
      }
