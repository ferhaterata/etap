...
baz.entry <- list(
  name = "entry",
  insts = list(
    "retval.i <- 0.000000e+00",
    "x.addr.i <- 0.000000e+00",
    "y.addr.i <- 0.000000e+00",
    "a.i <- 0.000000e+00",
    "x.addr <- 0.000000e+00",
    "y.addr <- 0.000000e+00",
    "z <- 0.000000e+00",
...
baz.for.cond <- list(
  ...
  insts = list(
    "v0 <- i",
    "cmp <- v0 - 3; if (is.numeric(cmp)) cmp <- DiscreteDistribution(cmp); cmp <- p(cmp)(-1)",
...
baz.for.body <- list(
  ...
  insts = list(
    ...
    "cmp.i <- v3 - add.i; if (is.numeric(cmp.i)) cmp.i <- DiscreteDistribution(cmp.i); cmp.i <- p(cmp.i)(-0.01)",
...
baz.if.then.i <- list(
  ...
  insts = list(
    ...
    "a.i <- mul.i",
    "v6 <- a.i",
    "cmp1.i <- v6 - -2.000000e+00; if (is.numeric(cmp1.i)) cmp1.i <- DiscreteDistribution(cmp1.i); cmp1.i <- 1 - p(cmp1.i)(0.01)",
...
edge_global <- as.list(rep(0, 12))
edge_local <- as.list(rep(0, 12))
edge_visited <- as.list(rep(0, 12))
names(edge_global) <- c("entry#for.cond", "for.cond#for.body", "for.cond#for.end", "for.body#if.then.i", "for.body#if.end4.i", "if.then.i#if.then2.i", "if.then.i#if.end.i", "if.then2.i#foo.exit", "if.end.i#if.end4.i", "if.end4.i#foo.exit", "foo.exit#for.inc", "for.inc#for.cond")
names(edge_local) <- c("entry#for.cond", "for.cond#for.body", "for.cond#for.end", "for.body#if.then.i", "for.body#if.end4.i", "if.then.i#if.then2.i", "if.then.i#if.end.i", "if.then2.i#foo.exit", "if.end.i#if.end4.i", "if.end4.i#foo.exit", "foo.exit#for.inc", "for.inc#for.cond")
names(edge_visited) <- c("entry#for.cond", "for.cond#for.body", "for.cond#for.end", "for.body#if.then.i", "for.body#if.end4.i", "if.then.i#if.then2.i", "if.then.i#if.end.i", "if.then2.i#foo.exit", "if.end.i#if.end4.i", "if.end4.i#foo.exit", "foo.exit#for.inc", "for.inc#for.cond")
...
# run the machine
DFS(list(), baz$blocks[[1]], "", "1", visited, edge_visited, env)

prob_global_file <- paste0(".output/", baz$name, ".global.prob")
...
prob_local_file <- paste0(".output/", baz$name, ".local.prob")
