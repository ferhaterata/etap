...
energy_trace <- read.csv("experiments/loop-overhead/.energytrace/EnergyTrace_2021_03_09_231958.csv", header = TRUE, sep = ",")
...
print_addressing_modes_time <- function() {
  cat("\nTime @ 1 MHz (ns)")
  cat("\nMode ", "Mean", "Sd")
  cat("\nLoop ", round(mean(block_time_dist_list[[1]]), 2), round(sd(block_time_dist_list[[1]]), 2))
}

print_addressing_modes_energy <- function() {
  cat("\nEnergy (uj)")
  cat("\nMode ", "Mean", "Sd")
  cat("\nLoop ", round(mean(block_energy_dist_list[[1]]), 2), round(sd(block_energy_dist_list[[1]]), 2))
}
